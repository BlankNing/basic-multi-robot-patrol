...
class CentralMonitor:
    ...
    def simulate(self):
        for t in self.time_series:
            ...
            for robot in self.robots:
                ...
                visited_node, v_flag = robot.visit_node(t, patrol_period)
                if v_flag:
            ...
            # Record maximum idleness for the current time step
            max_refresh_time = max([node.refresh_time for robot in self.robots for node in robot.patrol_nodes])
            self.max_idleness.append(max_refresh_time)
    ...
    def plot_max_idleness_over_time(self):
        plt.figure(figsize=(12, 6))
        plt.plot(self.time_series, self.max_idleness, label='Max Idleness')
        plt.xlabel('Time (s)')
        plt.ylabel('Max Idleness (s)')
...
# Plot max idleness over time
central_monitor.plot_max_idleness_over_time()
